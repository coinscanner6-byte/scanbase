...
from storage.db import load_countries, load_listed_symbols, load_supplies
...
def update_official_prices(collected, results, state, log):
    ...
    saved = save_indexes(indexes, now, state["listed"], state.get("supplies"))
    ...
    peg = indexes.get(("USDT", "USD"), {}).get("price")
    log(f"  official prices: {saved} ({saved - inr} USD, {inr} INR), "
        f"USDT-INR {usdt_inr}, USDT-USD {peg}")

    # Percentage changes, from the candles we already store.
    changed = update_changes(now)
    log(f"  changes: {changed} rows updated (1h, 24h, 7d)")
...
def main():
    ...
    last_fiat = None      # None = fetch the bank rate on the first round
    state = {}
    ...
    while True:
        ...
        # Every few hours: the ordinary bank USD-INR rate.
        # Not crypto data, and never allowed to stop collection.
        if last_fiat is None or started - last_fiat >= FIAT_REFRESH_HOURS * 3600:
            refresh_usd_inr(log=log)
            last_fiat = started
